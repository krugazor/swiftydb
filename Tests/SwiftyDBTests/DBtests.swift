...
class TestData : NSObject, Storable {
    ...
    public required init(storable: [String : Value]) {
       objectid = storable["objectid"] as! Int? ?? 0
       str = storable["str"] as! String? ?? ""
       date = storable["date"] as! Date? ?? Date()
    }
...
class DBTests : XCTestCase {
    ...
    func testLimitsAndOffsets() {
        ...
        XCTAssertLessThan((tenResults?.value?[0].objectid)!, (tenResults?.value?[9].objectid)!, "data is not ordered")
        ...
        XCTAssertEqual(twentyOffsetted?.value![0].objectid, 11, "expected 11, got " + String(describing: twentyOffsetted?.value![0].objectid))
    }
}
